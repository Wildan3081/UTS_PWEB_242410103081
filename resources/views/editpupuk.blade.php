...
<div class="max-w-3xl mx-auto bg-white shadow-lg rounded-xl p-8 mt-12">
    <h2 class="text-2xl font-bold text-emerald-700 mb-6">Edit Data Pupuk</h2>

    <form method="POST" action="{{ route('pupuk.edit', $pupuk['id']) }}">
        @csrf

        <div class="mb-4">
            <label class="block font-semibold text-gray-700 mb-2">Nama Pupuk</label>
            <input type="text" name="nama_pupuk" value="{{ $pupuk['nama_pupuk'] }}" class="w-full border rounded px-3 py-2" required>
        </div>

        <div class="mb-4">
            <label class="block font-semibold text-gray-700 mb-2">Penjelasan</label>
            <textarea name="penjelasan" class="w-full border rounded px-3 py-2" required>{{ $pupuk['penjelasan'] }}</textarea>
        </div>

        <div class="mb-4">
            <label class="block font-semibold text-gray-700 mb-2">Jumlah (Stok)</label>
            <input type="number" name="jumlah" min="1" value="{{ $pupuk['jumlah'] }}" class="w-full border rounded px-3 py-2" required>
        </div>

        <div class="mb-4">
            <label class="block font-semibold text-gray-700 mb-2">Harga (Rp)</label>
            <input type="number" name="harga" min="0" value="{{ $pupuk['harga'] }}" class="w-full border rounded px-3 py-2" required>
        </div>
